...
async def test_pending_cleared_after_ttl_for_same_symbol(monkeypatch):
    ...
    ok, reason, _ = await queue.put(_payload("BTC/USDT:USDT", INTENT_ENTRY))
    assert ok is True
...
async def test_scale_in_pending_cleared_after_ttl(monkeypatch):
    ...
    ok, reason, _ = await queue.put(_payload("ETH/USDT:USDT", INTENT_SCALE_IN))
    assert ok is True
...
async def test_priority_queue_ttl_smoke(monkeypatch):
    ...
    ok, _, _ = await queue.put(_payload("SMOKE/USDT", INTENT_ENTRY))
    assert ok is True
    t = 2.0  # advance beyond TTL
    ok, reason, _ = await queue.put(_payload("SMOKE/USDT", INTENT_ENTRY))
