"""
VST Canary-2: prove native trailing placement/activation (BingX swap, hedge mode).

This script makes real API calls against BingX VST (sandbox).

Goal:
  1) Open a tiny position (LONG or SHORT).
  2) Place exchange-native HARD_STOP via bot-integrated path (AdvancedPositionManager.open_position()).
  3) Enable trailing-stop (bot config) and prove the bot places a native TRAILING_STOP_MARKET after activation.
  4) Capture openOrders snapshots showing both conditionals.
  5) Demonstrate synthetic stop/trailing suppression signals (simulated via manage_position_exits()).

Artifacts written to diagnostics/vst/ (gitignored):
  - vst_canary2_trailing_activation_<ts>.json
  - vst_canary2_trailing_activation_<ts>.jsonl

Safety:
  - Forces TRADING_MODE=live, EXECUTION_BACKEND=ccxt, BINGX_ENV=vst.
  - Forces BINGX_NATIVE_HARD_STOP_ENABLED=true and BINGX_NATIVE_TRAILING_ON_ACTIVATION_ENABLED=true.
  - Requires hedge mode (dualSidePosition=true).
  - Always closes the position by MARKET at the end of the run (then closes locally + cancels conditionals).
...
logger = logging.getLogger("bingx_vst_canary2_trailing_activation")
...
class Canary2Summary:
    ...
    suppression_stop_loss: Dict[str, Any]
    suppression_trailing: Dict[str, Any]
...
async def _run(args: argparse.Namespace) -> Canary2Summary:
    ts_start = utc_now()

    out_dir = Path.cwd() / "diagnostics" / "vst"
    out_dir.mkdir(parents=True, exist_ok=True)
    ts_slug = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    jsonl_path = out_dir / f"vst_canary2_trailing_activation_{ts_slug}.jsonl"
    json_path = out_dir / f"vst_canary2_trailing_activation_{ts_slug}.json"
    ...
    signal = {
        ...
        "side": args.side,
        "exchange": "bingx",
        "strategy": "diagnostics_vst_canary2_trailing_activation",
        "stop": float(stop),
    ...
    logger.info(
        "[CANARY-2] Opening %s %s amt=%.8f (activation=%.4f%% delta=%.4f%%)",
        symbol,
        args.side,
        amount,
        float(args.trailing_activation_threshold_pct) * 100.0,
        float(args.trailing_distance_pct) * 100.0,
    )
    ...
    # Enable trailing and wait for native trailing placement.
    position_manager.configure_trailing_stop(
        position_id,
        enabled=True,
        delta_pct=float(args.trailing_distance_pct),
        activation_threshold_pct=float(args.trailing_activation_threshold_pct),
    )
    ...
    while True:
        ...
        ticker = exchange.fetch_ticker(symbol)
        last = _safe_float(ticker.get("last") or ticker.get("close") or (ticker.get("info") or {}).get("lastPrice"))
        await position_manager.monitor_position_pnl(position_id, current_price=last)
        position = position_manager.positions.get(position_id)
        trailing_order_id = (position or {}).get("native_trailing_stop_order_id") if isinstance(position, dict) else None
        _write_jsonl(jsonl_path, "pm.monitor_position_pnl", {"poll": polls, "last": last, "trailing_order_id": trailing_order_id})
        if trailing_order_id:
            logger.info("[CANARY-2] Native trailing placed. position_id=%s order_id=%s", position_id, trailing_order_id)
            break
        if (time.monotonic() - started) >= float(max(5.0, args.timeout_s)):
            raise RuntimeError("Timeout waiting for native trailing placement")
        await asyncio.sleep(float(max(0.5, args.poll_interval_s)))

    open_orders_trailing_raw = exchange.fetch_open_orders(symbol) or []
    ...
    # Synthetic suppression evidence (does not place market orders).
    position = position_manager.positions.get(position_id)
    suppression_stop_loss: Dict[str, Any] = {}
    suppression_trailing: Dict[str, Any] = {}
    if isinstance(position, dict):
        ...
        trailing_distance_val = _safe_float(position.get("trailing_stop_distance")) or float(args.trailing_distance_pct)
        entry_price = _safe_float(position.get("entry_price")) or (entry_avg or last)

        stop_hit_price = stop_loss * (0.99 if args.side == "long" else 1.01)
        position["exit_price"] = stop_hit_price
        suppression_stop_loss = await position_manager.manage_position_exits(position_id)
        _write_jsonl(jsonl_path, "suppression.stop_loss", {"exit_price": stop_hit_price, "result": suppression_stop_loss})
        ...
        suppression_trailing = await position_manager.manage_position_exits(position_id)
        _write_jsonl(jsonl_path, "suppression.trailing", {"exit_price": trail_hit_price, "result": suppression_trailing})

    # Close by MARKET for safety (use exchange-reported amount if available).
    close_amt = float((position_manager.positions.get(position_id) or {}).get("amount") or amount)
    close_request = {
        "symbol": symbol,
        "side": close_side,
        "amount": close_amt,
    ...
    _write_jsonl(jsonl_path, "order.market_close.result", {"close_result": close_result})
    if not close_result.get("success"):
        close_request["execution_params"] = {"reduceOnly": False, "positionSide": position_side}
        close_result = await order_manager.place_order(close_request, execution_algo="market")
        _write_jsonl(jsonl_path, "order.market_close.retry.result", {"close_result": close_result})
        if not close_result.get("success"):
            raise RuntimeError(f"Market close failed: {close_result.get('reason')}")
    ...
    positions_after_close_rest = _summarize_positions(client.get_bingx_positions(symbol))
    _write_jsonl(jsonl_path, "positions.after_close", {"positions": positions_after_close_rest})

    ok = bool(hard_stop_order_id and trailing_order_id)
    ts_end = utc_now()
    summary = Canary2Summary(
        ts_start=ts_start,
        ts_end=ts_end,
        ok=ok,
        side=args.side,
        symbol=symbol,
        amount=float(amount),
        trailing_activation_threshold_pct=float(args.trailing_activation_threshold_pct),
        trailing_distance_pct=float(args.trailing_distance_pct),
        ...
        positions_after_close_rest=positions_after_close_rest,
        suppression_stop_loss=suppression_stop_loss,
        suppression_trailing=suppression_trailing,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="BingX VST Canary-2: native trailing placement/activation evidence")
    ...
    ap.add_argument("--trailing-activation-threshold-pct", type=float, default=0.0)
    ap.add_argument("--trailing-distance-pct", type=float, default=0.002)
