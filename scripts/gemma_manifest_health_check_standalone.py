#!/usr/bin/env python3
"""
Standalone GEMMA Manifest Health Check

Validates manifest consistency without external dependencies.
"""
...
def check_manifest_existence(bundle_path: str) -> dict:
    ...
    try:
        ...
        result['status'] = 'pass'
        result['details'] = f"Manifest found and valid JSON"
...
def check_model_files_existence(manifest: dict, bundle_path: str) -> dict:
    ...
    for file_key in optional_files:
        if file_key in manifest:
            ...
            if not file_path.exists():
                ...
                abs_path = Path(manifest[file_key])
    ...
    if missing:
        ...
        result['status'] = 'warn'
        result['details'] = "No model files specified in manifest"
...
def print_check_result(check: dict):
    ...
    if 'found_files' in check and check['found_files']:
        print(f"   Found: {', '.join(check['found_files'])}")
