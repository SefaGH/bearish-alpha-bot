...
class ProductionCoordinator:
    ...
    async def _process_trading_loop(self):
        ...
        # Döngü sırasında değişme ihtimaline karşı stratejilerin bir kopyasını al
        ...
        for symbol in self.active_symbols:
            ...
            # Her sembol için ML context'i bir kez al
            ml_context = None
            ...
            processed_count += 1
            # Her stratejiyi bu sembol için çalıştır
            for strategy_name, strategy_instance in strategies_to_run:
                try:
                    if not self.portfolio_manager.is_strategy_active(strategy_name):
                        logger.debug(f"Skipping inactive strategy: {strategy_name}")
                        continue
                    
                    # Stratejiden sinyal üretmesini iste
                    signal = await strategy_instance.generate_signal(symbol, ml_context)
                    
                    # Eğer bir sinyal üretildiyse, onu işlenmesi için StrategyCoordinator'a gönder
                    if signal:
