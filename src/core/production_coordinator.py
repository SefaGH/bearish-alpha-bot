...
class ProductionCoordinator:
    ...
    async def _process_trading_loop(self):
        ...
        # StrategyCoordinator'ın kendisinin stratejileri çalıştırmasını tetikle
        if not self.strategy_coordinator:
            logger.error("❌ StrategyCoordinator is not available. Cannot process loop.")
            return
            
        # Artık stratejileri buradan almıyoruz, çünkü StrategyCoordinator yönetecek.
        # strategies_to_run = list(self.portfolio_manager.strategies.items())

        for symbol in self.active_symbols:
            try:
                logger.info(f"⚙️ [PROCESS] Delegating strategy execution for symbol: {symbol} to StrategyCoordinator")

                # --- DOĞRU YAKLAŞIM ---
                # StrategyCoordinator'a sembolü verip, tüm stratejileri
                # kendi içinde çalıştırmasını ve sinyal üretirse işlemesini istiyoruz.
                # Bu metodu StrategyCoordinator'a ekleyeceğiz.
                await self.strategy_coordinator.run_strategies_for_symbol(symbol)
                
                processed_count += 1

            except Exception as e:
                logger.error(f"❌ Error during delegated strategy execution for {symbol}: {e}", exc_info=True)
                error_count += 1
