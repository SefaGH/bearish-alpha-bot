...
class RLTradingEnv:
    """
    Gelişmiş RL Ticaret Ortamı (Final Version).
    
    Özellikler:
    1. Target Position Logic: Aksiyonlar hedef pozisyon oranını belirler.
    2. Reward Shaping: İşlem cezası (penalty) ve kırpma (clipping) uygulanır.
    3. State Augmentation: State vektörü, piyasa verilerine ek olarak portföy durumunu içerir.
    """
    ...
    def __init__(self, features_df: pd.DataFrame, raw_df: pd.DataFrame, config: Optional[Dict] = None, initial_balance: float = 10000.0):
        ...
        logger.info(f"✅ RLTradingEnv Initialized. Action: Target Position. State Dim: {self.state_dim}")
        self.reset()
    ...
    def _get_portfolio_value(self, price: float) -> float:
        """Yardımcı Fonksiyon: Güncel portföy değerini hesaplar."""
        return self.balance + (self.position * price)

    def _get_state(self) -> np.ndarray:
        """
        State vektörünü oluşturur.
        [Market Features (N)] + [Position Fraction (1)] + [Normalized PV (1)]
        """
        # 1. Market Features
        ...
        # Normalize edilmiş portföy değeri (Başlangıca göre oran)
        # Bu, ajanın ne kadar kâr/zararda olduğunu anlamasına yardımcı olur.
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        if trade_fraction > 1e-6 and portfolio_value_before > 0:
            if delta_fraction > 0: # ALIM
                ...
                notional_to_buy = min(notional_to_buy, self.balance) # Bakiye kontrolü
