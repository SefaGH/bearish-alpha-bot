...
def fetch_30m(client, symbol: str, limit: int = 1000) -> pd.DataFrame:
...
def simulate_long_nextbar(df: pd.DataFrame, tp_pct: float, sl_atr_mult: float | None, fallback_sl_pct: float | None) -> Dict[str, float]:
    # For each bar i, entry at close[i]; evaluate high/low of next bar for fills
    entries, pnls = 0, []
    df = df.dropna().copy()
    for i in range(len(df)-1):
        ...
        if sl_atr_mult is not None:
            ...
            # fallback (percentage stop); if missing, very wide so it's unlikely to hit
            sl_pct = float(fallback_sl_pct) if fallback_sl_pct is not None else 0.05
            sl = entry * (1 - sl_pct)

        # Next bar fill logic
        if lo <= sl:
            pnls.append(-(entry - sl)/entry)  # convert to pct distance
            entries += 1
            continue
        if hi >= tp:
...
def sweep_ob(df30: pd.DataFrame, grid: Dict[str, List[Any]], fallbacks: Dict[str, Any]) -> pd.DataFrame:
    ...
    for rsi_max, tp_pct, sl_atr_mult in itertools.product(grid["rsi_max"], grid["tp_pct"], grid["sl_atr_mult"]):
        # build a view of rows where signal would trigger
        # OversoldBounce signal condition: rsi <= rsi_max
...
def main():
    ...
    # allow override via YAML if present
    ...
    # fetch & indicators
    df30 = _df_from_ohlcv(client.ohlcv(symbol, timeframe="30m", limit=limit))
